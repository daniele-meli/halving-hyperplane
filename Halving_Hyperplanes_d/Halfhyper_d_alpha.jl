...
# PRE: V is a set of vertices defining a polytope in R^d and alpha the desired proportion.
...
function partitioning_equations(V,alpha)
    @assert alpha isa Union{QQFieldElem, Rational, Integer} "alpha must be rational (QQFieldElem or Rational), got $(typeof(alpha))"
    d = length(V[1])
    variables = ["h$i" for i in 1:d+1]
    R, vars = polynomial_ring(QQ, variables)
    S = fraction_field(R)
    ...
    for tuple in vol_loc
        ...
        p = numerator(fct)
        q = denominator(fct)
        f = denominator(alpha)*p - numerator(alpha)*area*q
...
#      and alpha_1,..., alpha_n are rational numbers.
...
function ham_sandwich(polytopes,alphas)
    ...
    L = [partitioning_equations(vertices_list[i], alphas[i]) for i in 1:length(vertices_list)]
    targets = [Float64(alphas[i] * areaofpolyhedra(vertices_list[i])) for i in 1:length(vertices_list)]
            ...
            for sol in points
               if sol in cc
                ...
                cuts = [Float64(negative_areaofcut(V, coeff)) for V in vertices_list]
                    if all(abs(cuts[i] - targets[i]) < 1e-6 for i in 1:length(polytopes))
                        push!(solutions, sol)
                    end
                end
            end
...
#     area_P = areaofpolyhedra(V1)
#     area_Q = areaofpolyhedra(V2)
...
#     target_P = alpha[1] * area_P
#     target_Q = alpha[2] * area_Q

#     println("Target (alpha1) area of P = ", target_P, " ≈ ", Float64(target_P))
#     println("Target (alpha2) area of Q = ", target_Q, " ≈ ", Float64(target_Q))

#     solutions = ham_sandwich([P, Q], alpha)
#     println("Ham sandwich solutions: ", solutions)

#     for (i, sol) in enumerate(solutions)
#         h0 = [QQ(rationalize(BigFloat(x))) for x in sol]

#         cut_P = negative_areaofcut(V1, h0)
#         cut_Q = negative_areaofcut(V2, h0)

#         println("\nSolution $i: h = (", h0[1], ", ", h0[2], "), t = ", h0[3])
#         println("  Area cut of P = ", Float64(cut_P),
#                 " (target = ", target_P, " ≈ ", Float64(target_P), ")")
#         println("  Area cut of Q = ", Float64(cut_Q),
#                 " (target = ", target_Q, " ≈ ", Float64(target_Q), ")")
#     end
# end
# main()
...
function main()
    ...
    area_P = areaofpolyhedra(V_bread)
    area_Q = areaofpolyhedra(V_salmon)
    area_T = areaofpolyhedra(V_cheese)
    ...
    # ===== Desired proportions per solid (must be rational: QQFieldElem / Rational / Integer) =====
    alpha = [QQ(1,2), QQ(1,3), QQ(2,3)]

    target_P = alpha[1] * area_P
    target_Q = alpha[2] * area_Q
    target_T = alpha[3] * area_T

    println("Target (alpha1 = ", alpha[1], ") volume of bread  = ", target_P, " ≈ ", Float64(target_P))
    println("Target (alpha2 = ", alpha[2], ") volume of salmon = ", target_Q, " ≈ ", Float64(target_Q))
    println("Target (alpha3 = ", alpha[3], ") volume of cheese = ", target_T, " ≈ ", Float64(target_T))

    @time solutions = ham_sandwich([P, Q, T], alpha)
    println("Ham sandwich solutions: ", solutions)

    for (i, sol) in enumerate(solutions)
        h0 = [QQ(rationalize(BigFloat(x))) for x in sol]

        cut_P = negative_areaofcut(V_bread, h0)
        cut_Q = negative_areaofcut(V_salmon, h0)
        cut_T = negative_areaofcut(V_cheese, h0)

        println("\nSolution $i: h = ", h0)
        println("  Volume cut of bread  = ", Float64(cut_P),
                " (target = ", target_P, " ≈ ", Float64(target_P), ")")
        println("  Volume cut of salmon = ", Float64(cut_Q),
                " (target = ", target_Q, " ≈ ", Float64(target_Q), ")")
        println("  Volume cut of cheese = ", Float64(cut_T),
                " (target = ", target_T, " ≈ ", Float64(target_T), ")")
    end
end
main()